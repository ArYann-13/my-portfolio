...
const Education = () => {
  ...
  return (
    <section id='education' className='relative  text-light-text dark:text-dark-text bg-light-background dark:bg-dark-background'>
        ...
        <div className="relative w-full max-w-3xl">
          ...
          <div className="space-y-12">
            {timelineEvents.map((event, index) => (
              <div
                key={index}
                className={`relative flex items-center w-full flex-col md:flex-row ${event.side === "left" ? "md:justify-start" : "md:justify-end"
                  }`}
              >

                <div
                  className={`hidden md:block absolute top-1/2 h-1 w-5/12 bg-gray-500 ${event.side === "left" ? "right-1/2" : "left-1/2"
                    }`}
                ></div>
                ...
                {/* Timeline Card */}
                <div
                  className={`w-full md:w-5/12 p-5 relative bg-white dark:bg-gray-800 rounded-lg shadow-lg transition-transform transform hover:scale-105 ${event.side === "left" ? "md:text-right md:right-16" : "md:text-left md:left-16"
                    }`}
                >
      ...
      <hr className='mt-16  border-t-2 border-[#700c86] w-3/4 md:max-w-full mx-auto' />
